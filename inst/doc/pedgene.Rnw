...
containing gene, chromosome, the kernel statistic and p-value, and the burden 
statistic and p-value.  The kernel p-value is calculated using Davies' method~\cite{Davies}, 
and the burden p-value is based on the normal distribution.
...
\noindent The results are now insignificant because we simulated
